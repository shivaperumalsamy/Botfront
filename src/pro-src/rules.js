    ...
    initEventHandler(rules, withViz = true) {
        ...
        trigger.eventListeners.forEach(listener => {
            ...
            if (elemList.length > 0) {
                elemList.forEach(elem => {
                    ...
                    if (
                        this.verifyConditions(rules, true) &&
                        listener.visualization !== 'none' &&
                        withViz
                    ) {
        ...
        this.resetListenersTimeouts.push({
            timeout: setTimeout(() => {
                ...
                window[RULES_HANDLER_SINGLETON].resetListenersTimeouts = window[
                    RULES_HANDLER_SINGLETON
                ...
                window[RULES_HANDLER_SINGLETON].eventListeners = window[
                    RULES_HANDLER_SINGLETON
                ].eventListeners.filter(
                    listener =>
                        !eventListenersForThisTrigger.some(
                            eListener => eListener.id === listener.id
                        )
                );
